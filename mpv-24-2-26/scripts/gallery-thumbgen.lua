...
local ytdl = {
    path = "yt-dlp",
    searched = false,
    blacklisted = {} -- Add patterns of URLs you want blacklisted from yt-dlp,
...
function ytdl_thumbnail_url(input_path)
    ...
    --otherwise proceed with the slower `yt-dlp -J` method
    if not (ytdl.searched) then --search for youtude-dl in mpv's config directory
        ...
        if not (ytdl_mcd == nil) then
            msg.error("found yt-dlp at: " .. ytdl_mcd)
            ytdl.path = ytdl_mcd
        end
        ytdl.searched = true
    end
    ...
    if (es < 0) or (json == nil) or (json == "") then
        msg.error("fetching thumbnail url with yt-dlp failed for" .. input_path)
        return input_path
    end
    local json, err = utils.parse_json(json)
    if (json == nil) then
        msg.error("failed to parse json for yt-dlp thumbnail: " .. err)
        return input_path
    end

    if (json.thumbnail == nil) or (json.thumbnail == "") then
        msg.error("no thumbnail url from yt-dlp.")
        return input_path
    end
